<template>
    <div v-if="listing" class="relative w-full" :class="listing.offerStage === 'Completed' ? 'opacity-40' : ''">
      ...
      <div
        ...
        <a @click="onLisitngClick(listing)" :href="localePath(listLink)" :target="isOwner" class="block w-full">
          <img v-if="imageUrl" :src="imageUrl" :alt="listing.offerName"
          ...
          <img v-else src="~/assets/images/uplaod-default-img.png" :alt="listing.offerName"
            class="bg-gray-300 object-cover max-h-[100%] max-w-[100%]  transition duration-200 ease-in group-hover:rounded-b-none hover:rounded-none mx-auto">
        </a>
      ...
      <div class="product-title w-full overflow-hidden px-1">
        ...
        <!-- <div v-if="listing.price" class="text-xs text-gray-500 mb-2 flex items-center">
            <span>Value</span>
        ...
        <div class="flex items-center mb-1 flex-col">
          <div class="w-full flex mb-1 items-center">
            <div v-if="listing.price"
              ...
              <div class="float-left">
                <div class="flex items-center">
                  <span v-if="listing.price && listing.price > listing.price"
                  ...
                  <span v-if="listing.price && listing.price > listing.price && checkOffPercentage(listing)"
            ...
            <!-- <div class="flex" v-if="listing.price && listing.price > listing.price">
                              <span   class="  text-[13px] block font-normal  pl-2 text-gray-400 line-through">
                              {{listing.price}}
                            </span>
  
                             <span class="text-[#72ad26] text-[13px] font-medium block ml-2">{{getDiscountPercentage(listing.price,listing.price)}}</span>
                          </div> -->
          </div>
      ...
      <a v-if="isMyListing && !listing.featured && listing.offerStage === 'Published' && eligibleForFeature && !listing.featuredWaitListing"
  ...
  export default Vue.extend({
    ...
    computed: {
      ...
      listLink() {
        //  return this.isDraftListing() ? `/listing/item?id=${this.listing.draftOfferId}` : `/p/${this.listing.seOId || this.slugify(this.listing.offerName)}/${this.listing.offerId || this.listing.oid}`
    ...
    methods: {
      ...
      onLisitngClick(lisitngDet) {
        ...
        if (lisitngDet && lisitngDet.featured) {
      ...
      checkIsAffiliated(product) {
        ...
        if (product && product.user && product.user.gintaaAffiliated) {
          return true
        } else {
          return false
        }
      ...
      checkOffPercentage(productDet) {
        ...
        let price = productDet?.price
        let diffValue = +productDet?.price - +productDet?.price
  
        //console.log("diffValue",diffValue)
        if (diffValue > 0) {
          return this.$cacculateoffpercentage(+productDet?.price, +productDet?.price)
