<template>
  <div class="relative w-full">
    <div class="block w-full relative add-quickview">
      <favourite
        v-if="!isMyListing && !listing.type"
        :listing="listing"
        @removeFromFav="removeFromFav"
      />
<div class="relative">
      <a
        v-if="getImageUrl(listing.image.url)"
      ...
      >
        <img
          v-if="getImageUrl(listing.image.url)"
          :src="getImageUrl(listing.image.url)"
          :alt="listing.name"
          class="
            bg-gray-300
            object-cover
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M12 7C10.8954 7 10 7.89543 10 9C10 10.1046 10.8954 11 12 11C13.1046 11 14 10.1046 14 9C14 7.89543 13.1046 7 12 7ZM8 9C8 6.79086 9.79086 5 12 5C14.2091 5 16 6.79086 16 9C16 11.2091 14.2091 13 12 13C9.79086 13 8 11.2091 8 9Z" fill="white"/>
      ...
      <div v-if="listing.type == 'StaticCoin'" class="p-2 h-[275px]">
        <div
          class="
            w-full
            flex
        ...
        >
          <img
            v-if="!getImageUrl(listing.image.url)"
            src="~/assets/images/gintaa-coin-bag.svg"
            alt="gintaa coin"
            class="
              max-h-[100%] max-w-[100%]
              transition
              ...
              ease-in
              group-hover:rounded-b-none
              hover:rounded-none gcon-bag
            "
          />
        </div>
        <div class="text-center">{{ listing.unitOfferValuation }}</div>

          <!-- <div class="flex" v-if="listing.price && listing.price > listing.unitOfferValuation">
                            <span   class="  text-[14px] block font-normal  pl-2 text-gray-400 line-through">
                            {{listing.price}}
                          </span>
                           <span class="text-[#72ad26] text-[14px] font-bold block ml-2">{{getDiscountPercentage(listing.unitOfferValuation,listing.price)}}</span>
                        </div> -->

      </div>

      <div v-if="listing.type == 'StaticCash'" class="p-2 h-[275px]">
        <div
          class="
            w-full
            flex
        ...
        >
          <img
            v-if="!getImageUrl(listing.image.url)"
            src="~/assets/images/gintaa-rupees-new-symbol.svg"
            alt="gintaa rupees"
            class="
              max-h-[100%] max-w-[100%]
              transition
      ...
      <div
        v-if="listing.name && !listing.type"
        class="product-title w-full overflow-hidden px-1"
      >
        <h2
          class="text-heading font-semibold truncate mb-3 text-sm text-gray-600"
        >
          {{ listing.name }}
        </h2>
          ...
          <div class="w-5/5 md:w-3/5">
            <div class="text-gray-500 text-sm font-normal flex item-center">
              ...
              <span class="flex items-center mr-1 ml-1 font-medium">
                ...
                <!-- <svg width="7" height="14" viewBox="0 0 11 18" fill="none" class="mr-1">
