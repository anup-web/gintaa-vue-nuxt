  ...
  <div class="">
    <aside>
      <div class="block border-b border-gray-200 pb-7 mb-7">
        ...
        <div v-show="selectedFiltersLength > 0" :class="[showMoretags ? '' : 'max-h-28 overflow-hidden', '']">
          <div v-for="filterObject of sortFilObjet(filterObjects)" :key="filterObject.name" class="flex flex-wrap -m-1.5 pt-2">
             ...
             <fieldset v-if="filterObject.type ==='dropdown'">
              ...
              <div class="pt-1 space-y-3">
                <div class="relative">
                  ...
                  <ul v-show="filterObject.showFilter" class="absolute w-full py-1 mt-1 overflow-auto bg-white rounded-md shadow max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none text-sm">
                    <li v-for="filter of filterObject.filters" :key="filter.name" class="text-gray-500 text-sm py-2 pl-10 pr-4 cursor-pointer hover:bg-gray-100" @click="dropdownFilter(filterObject, filter); filterObject.showFilter = false">
    ...
    <div class="md:hidden left-0 bg-white shadow-2xl fixed bottom-0 p-4 border-t z-40 w-full text-center text-[15px] font-medium text-gray-600 flex justify-center items-center">
  ...
  methods: {
    ...
    getSearchParams () {
      ...
      for (const filterObj of this.filterObjects) {
        ...
        //     obj[filterObj.paramName[1]] = this.$moment().format('YYYYMMDD')
        //   }
        // }
      }
