<template>
  <div class="flex-shrink-0 pe-24 hidden md:block md:w-72 lg:w-80 xl:w-96">
    <aside>
      <div class="block border-b border-gray-200 pb-7 mb-7">
        ...
        >
          ...
          <div class="flex flex-wrap -m-1.5 pt-2">
            <div
              v-for="filter of getTotalObjLength(totalSlectedObj, showMoretags)"
              :key="filter.name"
            >
              <div
        ...
        <form class="">
          ...
          >
            ...
            >
              ...
              <div
                v-for="filter of removeBlankObject(filterObject.filters)"
                :key="filter.name"
                class="pb-custom"
              >
                <div
                  v-if="filterObject.type == 'checkbox'"
                  class="flex items-center pb-3 relative pl-4"
                >
                  <input
                    :id="'f-' + filter.name"
                ...
                <!-- <div
                  v-else-if="filterObject.type == 'radio'"
                ...
                <div
                  v-else-if="filterObject.type == 'date'"
                  class="flex items-center pb-3"
                >
                  <label
            ...
            >
              ...
              <div
                v-if="
                  filterObject &&
                  filterObject.selectedRange &&
                  filterObject.selectedRange.length
                "
...
export default {
  ...
  props: ["filterObjects", "appliedFilters", "initialFilterObj"],
  data() {
    return {
      showMoretags: false,
      value: [20, 40],
  ...
  computed: {
    selectedFiltersLength() {
      //If condition Added to Avoid Mapping Issue
  ...
  methods: {
    ...
    removeSortParam(filterobj){
      filterobj.filter((item) => item.paramName !== "sort");
    },
    ...
    removeBlankObject(unFilteredObj) {
      const filteredObj = unFilteredObj.filter(
      ...
      for (const filterObj of this.filterObjects) {
        ...
        if (selectedFilter.length > 0) {
          if (filterObj.type === "checkbox") {
            ...
            this.cateSelectedGory();
            //  obj.push(`${filterObj.paramName}${selectedFilter.join('|')}`)
          }
        }
      }
