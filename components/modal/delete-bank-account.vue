    ...
    <template>
  <div
    class="fixed hidden bg-gray-800/[.5] z-40 inset-0 overflow-y-hidden"
    id="delete-modals"
      ...
      >
        ...
        <div class="w-full flex justify-center items-center pt-3 px-6 mt-1">
          <div
            class="
              rounded-full
              border border-dotted border-red-600
              w-12
              h-12
              flex
              justify-center
        ...
        <div class="w-full flex justify-center items-center pt-1 px-6 mt-3">
          <div class="text-sm text-gray-500 text-center">
            Are you sure you want to delete selected bank account 
          </div>
        </div>
...
export default Vue.extend({
  ...
  mounted() {
    console.log("showmodal", this.deletedItemDetails);
    if (this.deletedItemDetails.showdltbankAcntmodal) {
  ...
  methods: {
    cancelDelete() {
     ...
     closeModalWithRes(){
      ...
      this.$parent.$emit("closeModal", "BankAccount");
    },
    closeModal() {
      let modal = document.getElementById("delete-modals")!;
      modal.style.display = "none";
    ...
    async confirmDelete() {
      this.loading = true;
      try {
        let url =  `/users/v1/user/fund-account/id/${this.deletedItemDetails.fundAccountId}`
