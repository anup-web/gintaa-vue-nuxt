  ...
  <div class="">
    ...
    <form @submit.prevent="updateAddressInfo()">
      ...
      <div class="w-full mx-auto flex flex-col justify-center " >
        <div class="flex flex-col" v-bind:class="{ ' h-[380px] pt-3 overflow-auto auto-scroll': isMode(mode) }" >
          ...
          <div class="flex flex-col">
            ...
            <div class="relative z-0 w-full mb-6">
              <input
                ...
                id="flatNo"
                name="flatNo"
                v-on:input="onChange(address.flatNo, 'flatNo')"
                placeholder=" "
...
export default Vue.extend({
  ...
  methods: {
    ...
    async addAddress(addObj: any) {
      if (addObj) {
        this.loading = true;
        const requestBody = JSON.parse(JSON.stringify(addObj));
        console.log(requestBody)
        requestBody.annotation = requestBody.annotation === "Other" ? requestBody.annotationOther : requestBody.annotation;
        ...
        if (requestBody.addressId) {
          try {
            let url = `/users/v1/user/address/${requestBody.addressId}`;
            const editAddress = await this.$axios.$put(url, requestBody);
            if (editAddress.success) {
              this.$emit("editAddress", editAddress);
            } else {
            }
            this.loading = false;
          } catch (error: any) {
          ...
          try {
            let addAddress = await this.$axios.post(
              this.apiUrls.userAddress,
              requestBody
            );
            this.loading = false;
           // console.log("addAddress:", addAddress);
            this.$emit("saveAddress", addAddress);
          } catch (error: any) {
            console.log(error);
            this.loading = false;
          }
    ...
    async onChange(fieldVal: any, fieldName: any) {
      switch (fieldName) {
        ...
        case "addressLine":
          ...
          case "flatNo":
          if (!fieldVal) {
            this.errors.flatNo = this.$t('flatNoreq'); 
          } else {
            this.errors.flatNo = "";
          }
          // code block
