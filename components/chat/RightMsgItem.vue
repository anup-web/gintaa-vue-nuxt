<template>
  <div class="w-full mb-4 px-4 flex justify-end ">
    <div class="max-w-[86%] flex justify-end">
      <div class="max-w-full relative bg-[#E8F4D8] rounded p-2 msg-cotainer mgs-before-right break-words">
        <div v-if="message.messageType == 'HTML'" class="w-full text-xsb text-[rgba(0, 0, 0, 0.9)] ">
          <ChatReplyViewRight v-if="message.replyObj" :message="message" />
        ...
        <div v-else-if="message.messageType == 'IMAGE'" class="send-image-view overflow-hidden w-64">
          ...
          <div class="w-full flex justify-start text-xsb text-[rgba(0, 0, 0, 0.9)] ">
        ...
        <div v-else-if="message.messageType == 'AUDIO_RECORDING'" class="w-full text-xsb text-[rgba(0, 0, 0, 0.9)] ">
          <ChatReplyViewRight v-if="message.replyObj" :message="message" />
        ...
        <div v-else-if="message.messageType == 'FILE'" class="w-full ">
          ...
          <a :href="message.messageAttr.mediaUrls[0]" target="_blank" class="flex items-start cursor-pointer">
            ...
            <span class="text-xsb text-[rgba(0, 0, 0, 0.9)] pl-2 overflow-hidden">{{ message.messageBody | truncate(150) }}</span>
          </a>
        </div>

        <div v-else-if="message.messageType == 'OFFER'" class="w-60">
          <ChatReplyViewRight v-if="message.replyObj" :message="message" />
          <a :href="getLink" target="_blank" class="group box-border overflow-hidden flex p-3  cursor-pointer flex-col items-start bg-white shadow relative">
        ...
        <div v-if="message.messageBody.length > 150" class="w-full flex justify-end">
          <div v-if="!readMore" class="flex items-center text-xs text-gray-300 cursor-pointer" @click="readMore = true">
            <span class="pr-1"> Read more </span>
            <svg width="8" height="6" viewBox="0 0 14 8" fill="none">
              <path d="M0.8 1.7L7 8L13.2 1.7C13.4 1.5 13.5 1.2 13.5 1C13.5 0.8 13.4 0.5 13.2 0.3C13 0.1 12.8 0 12.5 0H1.5C1.2 0 1 0.1 0.8 0.3C0.6 0.5 0.5 0.7 0.5 1C0.5 1.3 0.6 1.5 0.8 1.7Z" fill="rgb(209 213 219)" />
            </svg>
          </div>
          <div v-if="readMore" class="flex items-center text-xs text-gray-300 cursor-pointer" @click="readMore = false">
            <span class="pr-1"> Read less  </span>
            <svg width="8" height="6" viewBox="0 0 13 8" fill="none">
              <path d="M12.7 6.3L6.5 0L0.3 6.3C0.1 6.5 0 6.8 0 7C0 7.2 0.1 7.5 0.3 7.7C0.5 7.9 0.7 8 1 8H12C12.3 8 12.5 7.9 12.7 7.7C12.9 7.5 13 7.2 13 7C13 6.8 12.9 6.5 12.7 6.3Z" fill="rgb(209 213 219)" />
            </svg>
          </div>
        </div>

        <div class="w-full flex justify-end msg-time-bottom ">
          <div class="text-xs text-[rgba(0, 0, 0, 0.7)] flex self-end mr-2">
            {{ $moment(message.messageTime).format('hh:mm A') }}
          </div>

          <div class="double-check flex self-end">
            <svg v-if="message.isRead" width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            <svg v-else-if="message.isDelivered" width="24" height="24" viewBox="0 0 24 24" fill="none">
